...
def get_discrete_explanation_topK(score_explanation, topK, only_pos=False):
    ...
    topk_score_explanations = [1 if i in topk_indices else 0 for i in range(len(score_explanation))]
    return topk_score_explanations


def _check_and_define_get_id_discrete_rationale_function(based_on, fx=None, baseline=None):
    if based_on == "th":
        get_discrete_rationale_function = _get_id_tokens_greater_th
    elif based_on == "k":
        get_discrete_rationale_function = _get_id_tokens_top_k
    elif based_on == "perc":
        # get_discrete_rationale_function = partial(_get_id_tokens_percentage, fx=fx, baseline=baseline)
...
def parse_evaluator_args(evaluator_args):
    ...
    removal_args = {
        ...
        "thresholds": np.array(evaluator_args.get("thresholds", np.arange(0.1, 1.1, 0.1))),
    }
