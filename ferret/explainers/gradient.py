    ...
    def compute_feature_importance(
        ...
        **explainer_args,
        ...
        def func(input_embeds):
            outputs = self.helper.model(inputs_embeds=input_embeds, attention_mask=item["attention_mask"])
            return outputs.logits

        dl = InputXGradient(func, **init_args) if self.multiply_by_inputs else Saliency(func, **init_args)
...
class IntegratedGradientExplainer(BaseExplainer):
    ...
    def _generate_baselines(self, input_len):
        ...
        embeddings = self.helper._get_input_embeds_from_ids(torch.tensor(ids, device=self.device))
        return embeddings.unsqueeze(0)

    def compute_feature_importance(self, embeds, target, **explainer_args):
        init_args, call_args = parse_explainer_args(explainer_args)
        # item = self._tokenize(embeds)
        # input_len = item["attention_mask"].sum().item()
        ...
        def func(input_embeds):
            # attention_mask = torch.ones(*input_embeds.shape[:2], dtype=torch.uint8, device=self.device)
            _, logits = self.helper._forward_with_input_embeds(
                input_embeds, show_progress=show_progress
                # input_embeds, attention_mask, show_progress=show_progress
            )
            return logits

        dl = IntegratedGradients(func, multiply_by_inputs=self.multiply_by_inputs, **init_args)
        # inputs = self.get_input_embeds(embeds)
        # pred = func(inputs)
        baselines = torch.zeros_like(embeds)
        # baselines = self._generate_baselines(input_len)

        attr = dl.attribute(embeds, baselines=baselines, target=target, **call_args)
