...
def lp_normalize(explanations, ord=1):
    ...
    for exp in explanations:
        ...
        new_exp.scores /= np.linalg.norm(exp.scores, axis=-1, ord=ord)  # L1 normalization
...
class Benchmark:
    ...
    def __init__(
        ...
        if not explainers:
            self.explainers = [
                ...
                IntegratedGradientExplainer(self.model, self.tokenizer, multiply_by_inputs=False),
                IntegratedGradientExplainer(self.model, self.tokenizer, multiply_by_inputs=True),
            ]
        ...
        if not evaluators:
            ...
            self.evaluators = [ev(self.model, self.tokenizer) for ev in self._used_evaluators]
        if not class_based_evaluators:
            ...
            self.class_based_evaluators = [
                class_ev(self.model, self.tokenizer) for class_ev in self._used_class_evaluators
            ]

    def explain(
        self, text, target=1, show_progress: bool = True, normalize_scores: bool = True, order: int = 1, baseline=None
    ) -> List[Explanation]:
        ...
        explanations = list()
        # if not isinstance(self.explainers[0], SHAPExplainer):
        #     _explainers = list()
        #     shap = None
        #     for exp in self.explainers:
        #         if not isinstance(self.explainers[0], SHAPExplainer):
        #             _explainers.append(exp)
        #         else:
        #             shap = exp
        #     if shap is None:
        #         raise Exception(
        #             "Shap explainer must be given (for now) to extract baseline for faithfulness evaluation in regresson tasks."
        #         )
        #     self.explainers = [shap] + _explainers

        base_value = None

        for exp in self.explainers:
            if isinstance(self.explainers[0], SHAPExplainer):
                res = exp(text, target, baseline=baseline)
            else:
                res = exp(text, target)
            if base_value is None:
                base_value = res.base_values
            else:
                res.base_values = base_value
    ...
    def evaluate_explanation(
        ...
        **evaluation_args,
        ...
        for evaluator in self.evaluators:
            evaluation = evaluator.compute_evaluation(explanation, target, **evaluation_args)
            if evaluation is not None:  # return None for plausibility measure if rationale is not available
        ...
        if class_explanation is not None:
            for class_based_evaluator in self.class_based_evaluators:
                class_based_evaluation = class_based_evaluator.compute_evaluation(class_explanation, **evaluation_args)
    ...
    def evaluate_explanations(
        ...
        **evaluation_args,
        ...
        class_explanations_by_explainer = self._get_class_explanations_by_explainer(class_explanations)
        if show_progress:
    ...
    def score(self, text: str, return_dict: bool = True):
        ...
        if return_dict:
            scores = {self.model.config.id2label[idx]: value.item() for idx, value in enumerate(scores)}
        return scores
    ...
    def show_table(
        ...
        return (
            table.style.background_gradient(axis=1, cmap=SCORES_PALETTE, vmin=-1, vmax=1).format("{:.2f}")
            if apply_style
    ...
    def show_evaluation_table(
        ...
        for explanation_evaluation in explanation_evaluations:
            explainer_scores[explanation_evaluation.explanation.explainer] = {
                evaluation.name: evaluation.score for evaluation in explanation_evaluation.evaluation_scores
            }
    ...
    def _style_evaluation(self, table: pd.DataFrame) -> pd.DataFrame:
        ...
        table_style = table.style.background_gradient(axis=1, cmap=SCORES_PALETTE, vmin=-1, vmax=1)
        ...
        **evaluation_args,
        ...
        if is_thermostatdata:
            ...
            if target is None:
                ...
                predicted_classes = [self.score(i["text"], return_dict=False).argmax(-1).tolist() for i in instances]
        ...
        if n_workers > 1:
            ...
            for instance, target in zip(instances, targets):
                ...
                for explanation in explanations:
                    ...
                    evaluation = self.evaluate_explanation(explanation, target, show_progress=False, **evaluation_args)
    ...
    def show_samples_evaluation_table(
        ...
        table = pd.DataFrame(
            {
                explainer: {evaluator: mean_std[0] for evaluator, mean_std in inner.items()}
