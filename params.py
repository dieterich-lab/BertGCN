...
def parse_args(*args):
    ...
    parser.add_argument(
        "--noarznei",
        action="store_true",
        help="Trains the model without the sequence prefix `Arznei [...] &`.",
    )
    parser.add_argument(
        "--interpret_mode",
        type=str,
    ...
    parser.add_argument(
        "--bertmodel", type=str, default="medbert", choices=["gbert", "medbert"]
    )
    parser.add_argument(
        "--doclevel",
        type=str,
    ...
    parser.add_argument(
        "--gpus", "-g", type=int, nargs="+", default=[0], choices=[0, 1, 2, 3]
    )
    ...
    parser.add_argument(
        "--checkpoint", type=int, help="Load model from a specific checkpoint."
    )
    parser.add_argument(
        "-a", "--accelerator", default="gpu", type=str, choices=["gpu", "cpu"]
    )
    parser.add_argument(
        "-s",
        "--simple",
        action="store_true",
        help="Uses simplified transformations for rnn and trans.",
    )
    parser.add_argument(
        "--testunklar",
        action="store_true",
    ...
    parser.add_argument(
        "--suppressupdates",
        action="store_true",
        help="If true, updating graph features is suppressed.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--optimize",
        action="store_true",
        help="Use optuna library to grid search the best mixfactor.",
    )
    parser.add_argument(
        "--reportraw",
        action="store_true",
        help="Report raw (untokenized) dataset statistics.",
    )
    parser.add_argument(
        "--savebest",
        action="store_true",
        help="Only save the current checkpoint as best model.",
    )
    parser.add_argument(
        "--equalsets",
        action="store_true",
        help="Forces train, val and test set to be the same.",
    )
    parser.add_argument(
        "--sample", action="store_true", help="Sample from the data when in 'dev' mode."
    )
    parser.add_argument(
        "--noweightedsampling",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "--proceed",
        action="store_true",
        help="Continue training with current best model.",
    )
    parser.add_argument(
        "--allowtqdm", action="store_true", help="Show tqdm progressbar."
    )
    parser.add_argument(
        "--fromscratch",
        action="store_true",
        help="Trains the coding task with a freshly initialized model.",
    )
    parser.add_argument(
        "--cdscentered",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=1000,
        type=check_positive,
        help="Determines the maximum number of chunks.",
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="Coefficient for the penalization term.",
    )
